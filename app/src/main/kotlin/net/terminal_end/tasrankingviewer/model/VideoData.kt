...
sealed class VideoData {
    ...
    class Success(cmsId: String, title: String, tags: List<String>, startTime: Calendar, thumbnailUrl: String, viewCounter: Int, commentCounter: Int, myListCounter: Int): VideoData() {
        val cmsId = cmsId
        ...
        companion object {
            fun convertFromChunkHitsValues(values: List<Chunk.Value.Hits>): Result<List<VideoData.Success>, Exception> {
                return Result.Success(values.map {
                    if (it.cmsid != null || it.title != null || it.tags != null || it.start_time != null || it.thumbnail_url != null ||
                        ...
                        when (startTimeCalendar) {
                            is Result.Success -> {
                                VideoData.Success(it.cmsid!!, it.title!!, it.tags!!.split(" "), startTimeCalendar.value, it.thumbnail_url!!, it.view_counter!!, it.comment_counter!!, it.mylist_counter!!)
                            }
                            is Result.Failure -> {
